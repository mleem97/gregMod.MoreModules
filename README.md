# gregMod.MoreModules

> gregMod.MoreModules** adds faster, color-coded QSFP modules to the Data Center shop. Modules use the vanilla QSFP+ form factor and persist when installed in swi

![License](https://img.shields.io/github/license/mleem97/gregMod.MoreModules?style=for-the-badge) ![Last commit](https://img.shields.io/github/last-commit/mleem97/gregMod.MoreModules?style=for-the-badge) ![Repo size](https://img.shields.io/github/repo-size/mleem97/gregMod.MoreModules?style=for-the-badge) ![Stars](https://img.shields.io/github/stars/mleem97/gregMod.MoreModules?style=for-the-badge)

## Links

- **Steam Workshop:** [My Workshop (Data Center)](https://steamcommunity.com/id/frikadelle3000/myworkshopfiles/?appid=4170200)
- **Repository:** [https://github.com/mleem97/gregMod.MoreModules](https://github.com/mleem97/gregMod.MoreModules)
- **Issues:** [https://github.com/mleem97/gregMod.MoreModules/issues](https://github.com/mleem97/gregMod.MoreModules/issues)
- **Releases:** [https://github.com/mleem97/gregMod.MoreModules/releases](https://github.com/mleem97/gregMod.MoreModules/releases)

## Overview

**gregMod.MoreModules** — gregMod.MoreModules** adds faster, color-coded QSFP modules to the Data Center shop. Modules use the vanilla QSFP+ form factor and persist when installed in swi

See [docs/INDEX.md](docs/INDEX.md) for the complete documentation.

## Compatibility

| Platform | Status |
|---|---|
| Windows x64 | Supported |
| Linux x64 | Supported |

## Features

- See [docs/INDEX.md](docs/INDEX.md) and [QUICKSTART.md](QUICKSTART.md)

## Installation

See [QUICKSTART.md](QUICKSTART.md).

## Build from Source

```bash
git clone git@github.com:mleem97/gregMod.MoreModules.git
cd gregMod.MoreModules
```

Details: [QUICKSTART.md](QUICKSTART.md), [CONTRIBUTING.md](CONTRIBUTING.md).

## Repository Layout

```
├── README.md            # Diese Datei
├── QUICKSTART.md        # Schnellstart
├── CHANGELOG.md         # Changelog (Keep a Changelog)
├── CONTRIBUTING.md      # Mitmachen
├── SECURITY.md          # Sicherheitsmeldungen
├── CODE_OF_CONDUCT.md   # Verhaltenskodex
├── AGENTS.md            # Hinweise für KI-Agenten
├── LICENSE              # Apache-2.0
├── VERSION              # Single Source of Truth für die Version
├── docs/                # Dokumentation ([Index](docs/INDEX.md))
├── scripts/             # Build-/Hilfsskripte
├── tests/               # Tests
├── references/          # Referenzen
├── sponsors/            # Sponsoren
└── examples/            # Beispiele
```

## API Documentation

See [`docs/INDEX.md`](docs/INDEX.md).

## Credits

| Role | Contributor |
|---|---|
| **Codebase** | [mleem97](https://github.com/mleem97) |

## Contributing

See [CONTRIBUTING.md](CONTRIBUTING.md).

## License

Apache-2.0 — see [`LICENSE`](LICENSE).

## 🚀 Join the gregFramework Team!

Do you enjoy building mods, tools, or docs? Get in touch: **apply@gregframework.eu** or via
[Discord](https://discord.gg/greg) — Code, Assets, Docs, Testing, Infra, Community.

---

**gregFramework — powered by the community.**

